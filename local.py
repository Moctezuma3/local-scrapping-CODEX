...
def scrape_page(page, business_list: BusinessList):
    try:
        ...
        if listings_container.count() > 0:
            listings = listings_container.locator(
                'div.SearchResultList_listElementWrapper__KRuKD'
            ).all()
            ...
            for listing_index, listing in enumerate(listings):
                try:
                    ...
                    if listing.locator(address_selector).count() > 0:
                        address_element = listing.locator(address_selector).first
                        full_address = address_element.inner_text()

                        spans = address_element.locator('span').all()
                        for span in spans:
                            span_text = span.inner_text()
                            full_address = full_address.replace(span_text, '')

                        (
                            business.address,
                            business.postal_code,
                            business.city,
                        ) = business_list.split_address(full_address)

                    # --- Website ---
                    website_selector = 'a.ListElement_link__LabW8'
                    if listing.locator(website_selector).count() > 0:
                        business.website = "local.ch/" + listing.locator(
                            website_selector
...
def main():
    ...
    if not search_term:
        sys.exit("⚠️ Aucun terme de recherche fourni")

    input_file_name = 'input.txt'
    input_file_path = os.path.join(os.getcwd(), input_file_name)
    if not os.path.exists(input_file_path):
        sys.exit(f"⚠️ Le fichier {input_file_name} est introuvable")
        ...
        try:
            for search_for_index, search_for in enumerate(search_list):
                ...
                try:
                    ...
                    page.get_by_test_id("home-stage-container").get_by_test_id(
                        "search-q-input"
                    ).click()
                    page.get_by_test_id("home-stage-container").get_by_test_id(
                        "search-q-input"
                    ...
                    print(f"Erreur recherche {search_for}: {e}")
                    continue
                ...
                while True:
                    ...
                    try:
                        next_button = page.get_by_role(
                            "button", name="Suivant", exact=True
                        )
                ...
                # --- Sauvegarde ---
                filename = f"local_ch_data_{search_for.replace(' ', '_')}"
                business_list.save_to_excel(filename)
                business_list.save_to_csv(filename)
                print(f"✅ Données sauvegardées pour : {search_for}")
